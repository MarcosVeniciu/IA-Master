...
| RF-002  | Listar aventuras em andamento em formato de card.                       | Alta       | Cada card mostra o título específico da aventura (`adventureTitle`), o título do cenário base (`scenarioTitle`) como subtítulo, gênero, uma barra de progresso visual, informação de "Last access" e um botão/ícone "Play". |
